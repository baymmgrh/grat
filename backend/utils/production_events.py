...
def register_production_events(app):
    ...
    def work_order_status_changed(mapper, connection, target):
        """
        Auto-create WIP Ledger and WIP Batch when Work Order status changes to 'in_progress'
        """
            ...
            if not existing_wip:
                ...
                with app.app_context():
                    try:
                        ...
                        # Get product
                        product = db.session.query(Product).get(target.product_id)
                        ...
                        print(f"✗ Failed to auto-create WIP Ledger: {str(e)}")
                        db.session.rollback()
            ...
            if not existing_batch:
                with app.app_context():
                    try:
                        ...
                        print(f"✗ Failed to auto-create WIP Batch: {str(e)}")
                        db.session.rollback()
